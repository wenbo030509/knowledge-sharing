...
## 十二、架构局限与升级路线（2026-08-20）

> 背景：百度 Agent 背后使用 Qwen / DeepSeek 开源模型（内部部署 API 调用），**未做业务后训练**，仅靠工程化 Harness 约束 + Prompt 规则约束。受限于业务成本与条件。
> 讨论产出：这个架构的合理性、天花板、三步升级路线与决策公式。

### 12.1 架构选择的合理性（为什么这样是对的）

```text
为什么可以不做后训练：
├── Agent 任务的难点不在"语言能力"，在"工具调用的正确性"
│   ├── 开源模型通用能力 + Function Calling 已足够强
│   └── Harness 约束解决 90% 输出形态问题
│       （Schema 约束、格式校验、流程编排、重试、回滚）
├── 后训练的真实成本：
│   ├── 高质量业务数据（标注+清洗+配比）
│   ├── GPU 训练资源 + 训练工程师排期
│   └── 评测验证闭环（训练完怎么证明变好了）
...
```text
第一步：评测闭环先补齐（零训练成本，收益最大）
├── 建立迁移 Badcase 评测集（成功/失败/人工修正案例）
├── 每次 Prompt 改动跑评测集 → 避免"修好一类引入另一类"
...
### 12.5 面试表达（版本 C 的追问弹药）

```text
面试官："你的 Agent 做了业务微调吗？"
你："没有，用的是内部部署的开源模型（Qwen/DeepSeek），
只有 Harness 和 Prompt 约束。这是成本约束下的正确选择——
92% 成功率已带来 98% 人力降本，业务价值未验证时不值得
投入后训练。但我清楚上限：Prompt 覆盖不到的 edge case
就失败，经验不沉淀。如果重新做，三步走：①Badcase 评测集
驱动 Prompt 迭代 ②从运行日志沉淀 SFT 数据，用 LoRA 做
轻量业务适配，Prompt 从'教规则'瘦身成'提示' ③评测驱动
持续迭代。核心洞察：训练数据可以来自 Agent 自己的运行
日志——这是数据飞轮。"
