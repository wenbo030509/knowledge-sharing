...
## 十、评测脆弱点归因与数据策略反馈（2026-08-23 追加）

> 性质：**前置归因**——基于评分标准 + 判分代码反推"评测规范层面的失败风险点"（未拿到真实判分结果时的结构分析）。拿到真实 bad case 后，按 projects/vlm-eval-feedback-loop.md 四类根因框架补充实证归因。
> 原则先置：**先查评测规范误伤，再归因数据问题**——评测规范不修，后面所有归因都会被污染。

### 10.1 判分代码 vs 评分规范的 gap（最高优先级：评测规范问题）

对照九章生成的判分代码与评分标准原文，发现 3 个可验证的判分误伤点：

```text
Gap 1：数字抽取只认阿拉伯数字（模式一）
├── 规范原文：M053 R7 accept '20', '20 points', 'twenty', '二十'
├── 代码实现：pick_integer 只 re.findall(r'\d+')，normalize 不转写英文/汉字数字
├── 后果：模型答 "twenty" / "二十" → 提取不到整数 → 0 分（尽管规范写了 accepted）
└── 判定：判分代码 vs 规范不一致 → 假失败（假 negative），污染归因

Gap 2：多数字场景取第一个（模式三）
├── 规范原文：M065 R15 干扰速度 253/210.8 判 0
├── 代码实现：grade_numeric_tolerance 用 re.search 取第一个数字
├── 后果：模型先提干扰速度再提正确答案 → 取到 253 → 0 分
└── 判定：规范意图是"识别目标速度"，实现是"第一个数字" → 容差类误伤

Gap 3：时间格式覆盖不全（模式三）
├── 代码实现：parse_mmss 只匹配 m:ss 格式
├── 后果：模型答 "52 seconds" / "52s" → 提取失败 → 0 分
└── 判定：输出格式变体未收口 → 格式误伤
```

**为什么这是第一优先**：这三类都是"答对了但判错"。在真实数据里它们会伪装成模型失败，导致误判模型能力短板 → 数据策略跟着错。

### 10.2 任务定义层风险（未成型 sheet + 模糊 rubric）

```text
风险 1：约半数 sheet 只有占位符/代码骨架
├── 涉及：M022 / M032 / M043 / M057 / M059 / M062 / M070 / M071 /
│         01_Productivity 系列 / 04_Search_Retrieval 系列
├── 后果：任务定义是开放空间 → 判分规则未写死 → 一致性不可控
└── 动作：按 5 部件框架（任务对象/证据边界/输出格式/容错规则/排除项）补齐

风险 2：视觉识别类 rubric 模糊（M099 / M101）
├── 原文：M099 "Evaluate whether the assistant correctly identified
│         the car and provided a plausible price range"
├── 后果：从 Deterministic Floor 退化为开放判断 → 依赖 LLM judge/人工
├── 已知代价：LLM judge 有位置/冗长/自我增强偏差（papers/05）
└── 动作：拆解为可判子项（车型名匹配 + 价格区间数值容差 + 价格单位）
```

### 10.3 公开论文支撑：这些失败模式的行业共性

| 任务类型 | 对应 sheet | 公开发现（已有精读笔记） | 归因方向 |
|---------|-----------|------------------------|---------|
| 长视频字幕/证据边界 | M015/M016/M046 | Video-MME 字幕轨 = 证据边界设计；长视频信息密度高、跨段推理是 VLM 共性短板 | 评测规范已尽力，失败多为模型能力 → 数据难度分级 |
| 计数/感知 | M048/M053/M055 | VLM 密集场景计数系统性失败（Fable 5 计数 3/10、BabyVision 不如 6 岁儿童） | 模型能力短板 → 数据策略：难度分级样本 |
| 时间定位/跨事件 | M016/M046/M064 | LongVT/VideoTemp-o3 的 locate-crop-answer 范式：时间交叉引用、实体匹配是 2026 数据构造主战场 | 模型能力短板 → 数据策略：补时间交叉引用样本 |
| 体育记分板 OCR | M034/M035/M053/M065 | 小字、动态刷新、overlay 遮挡是 OCR 难点；distractor 设计（84拍 overlay）正是为此 | 评测规范已处理干扰项；剩余失败多为 OCR 能力 |
| 结构化输出 | M035/M060/M065 | 格式敏感场景：模型"内容对但格式错"是常见失败形态 | 评测规范问题 → 部分分/格式收口（见 10.1） |

**归因结论（前置推断）**：失败大概率集中在两处——① 判分代码误伤（10.1 三类，先修）；② 模型真实能力短板（计数/时间定位/OCR），这两类要分开，不能混着写进 bad case 报告。

### 10.4 数据策略反馈报告草稿（给算法/研发，一页纸）

```text
# VLM 任务2 评测 第 0 轮 策略反馈（前置分析草稿）

## 本轮概况
- 判分结果：待回传（本轮为规则结构分析，无实证数据）
- 分析对象：39 sheet 评分标准 + 判分代码形态

## 根因与建议（按优先级）
├── 根因 A（评测规范问题，最高优先）
│   ├── 现象：判分代码与规范存在 3 处 gap
│   │   （数字转写缺失 / 多数字取第一个 / 时间格式不全）
│   ├── 建议：判分代码与规范对齐（补数字转写、多候选打分、格式收口）
│   └── 预期影响：消除"答对但判错"的假失败，后续归因才可信

├── 根因 B（任务定义问题）
│   ├── 现象：约半数 sheet 只有占位符；M099/M101 rubric 模糊
│   ├── 建议：按 5 部件框架补齐；模糊类拆解为可判子项
│   └── 预期影响：判分一致性提升，LLM judge 偏差面收窄

├── 根因 C（模型能力短板——不是 bug，是能力缺口）
│   ├── 现象（行业共性）：计数/时间定位/跨段推理是 VLM 2026 共性短板
│   ├── 建议：此类 bad case 单独归类（能力维度），数据策略走难度分级
│   └── 预期影响：能力短板 → 数据加样，而不是改评测

## 下轮主变量（只调 1 个）
- 修根因 A（判分代码与规范对齐）——评测规范问题不修，归因全被污染

## 需要算法侧配合
- 回传真实判分结果（按 sheet 维度）
- 确认 10.1 三处 gap 是否真实存在于生产判分代码
```

### 10.5 拿到真实判分结果后的归因路径

```text
1. 先跑"评测规范体检"：把 10.1 的 gap 测试（twenty/多数字/时间格式）
   在真实答案上跑一遍 → 量化假失败率
2. 再做四类根因分类：任务定义 / 数据源 / 标注执行 / 评测规范
3. 能力类 bad case 单独归类（连接公开论文对照表）
4. 按 vlm-eval-feedback-loop.md 填归因报告 + 验证记录表
```

### 10.6 公开论文案例库（相似案例扩展，2026-08-23 追加）

> 内部反馈涉密不可得，以下用公开论文/工具链案例佐证 10.1–10.4 的每个判断——这些案例证明：**"解析器造成假失败"是行业普遍现象，不是个例**。可直接引用到策略反馈报告中作外部依据。

#### 案例 1：解析器不同，同一模型分数从 0 到 0.95（佐证 10.1 三个 gap）

```text
来源：verl 框架 GSM8K 解析器对比（GitHub issue #2150）
数据：
├── Qwen2.5-Math-7B-Instruct：strict 提取 0.0 → flexible 0.919 → math_verify 0.956
├── Qwen2.5-Math-1.5B：strict 0 → flexible 0.82 → math_verify 0.852
└── 结论：解析器选择比模型能力更影响分数

来源：EleutherAI lm-evaluation-harness（issue #2707、#3214）
数据：
├── Qwen2-7B strict-match 0.57 vs 模型卡 0.82（解析器只认 "The answer is x."）
├── flexible-extract 不归一化 "$29.00" vs GT "29"（货币符号未剥）
└── 同类问题：我们 Gap 1 只认阿拉伯数字 / Gap 3 只认 m:ss
    = 同一个病根：解析器与规范 accepted forms 不对齐

行业量化（36 模型 × 15 任务研究）：
├── 平均 1/5 的正确回答因格式问题被误判为错
├── GSM8K 12.3-12.7% 被拒；MMLU 3.2-8.7%；MATH 11.3-23.6%
└── 单一模型极端案例：63% 正确回答被规则匹配拒绝
```

→ **对我们 10.4 根因 A 的意义**：修判分代码与规范对齐不是"吹毛求疵"，是消除 12%+ 级别的系统误差。量化方法现成：拿真实答案跑 gap 测试，假失败率直接测出来。

#### 案例 2：格式合规是算术评测的主瓶颈（佐证"先修评测规范"的优先级）

```text
来源：Revealing Latent Reasoning: Format Compliance as the Primary Bottleneck
     in Small Language Model Arithmetic Evaluation（2025）
数据：
├── Llama-3-8B-Instruct：GSM8K 全测试集仅 0.23%，但 81.73% 输出含潜在答案
├── 加一句格式指令 → 71.57%（311 倍提升）
└── 论文命名该现象："Latent Answer Accessibility Gap"（三个架构平均 67.39 分差距）

关键警示（防止修过头）：
├── 把"格式合规"单独作为奖励 → 模型"看起来对但实际不对"
│   （format reward 1.0 但 accuracy 0）——格式应作 gate 而非独立 reward
└── 对应我们：accepted forms 收口是减少假失败，不能变成"格式好就得高分"
    → 部分分设计（模式四）是正确形态：内容子项独立计分
```

#### 案例 3：计数是 VLM 系统性短板，且瓶颈在解码不在感知（佐证 10.3 计数行）

```text
来源：BLINK benchmark（核心感知能力评测）
数据：
├── Counting 任务（120 样本）：人类 93.75%，随机 25%
├── MiniGPT-4-v2 10.83%（低于随机！）；OpenFlamingo-v2 21.67%
├── 最强模型（LLaVA-v1.6-34B / Gemini Pro / GPT-4V）也远低于人类
└── 结论：计数失败是模型能力短板，与评测设计无关

来源：TallyQA（28.8 万 QA 对，计数标准数据集）
数据：
├── GT 标签分布严重偏斜（0/1/2 小计数占多数）→ 简单题占比虚高分数
├── QwenVL / BLIP2：简单题尚可、复杂题（属性约束/对抗题）崩
├── LVLM-Count：专门方法只提升复杂题，简单题几乎无提升
└── 参考：CountBenchQA 用人工核验的 540 图修正偏斜与噪声

来源：Vision language models are blind（线性探针实验）
├── 视觉编码器里信息足够（能线性解出计数），但 LM 解码失败
└── 对数据策略的含义：加样本有用，但瓶颈是解码 → 数据难度分级 +
    RL 训练信号比盲目加量更对症（对应 L4 认知：评测驱动训练方向）
```

→ **对我们 10.4 根因 C 的意义**：M048/M053/M055 计数类的 bad case 先归"模型能力"，别急着改评测或归标注。

#### 案例 4：长视频时间推理是共性瓶颈，短视频数据补不回来（佐证 10.3 时间定位行）

```text
来源：Video-MME（900 视频，11s 至 1 小时）
数据：
├── Gemini 1.5 Pro：短 81.7% → 长 67.4%（掉 14+pp）
├── 字幕提升 +6.2%、音频 +4.3%，长视频上增益更大
└── 长视频掉分 = 信息密度 + 上下文窗口挤压（needle in a haystack）

来源：TempCore 帧敏感性分析
├── 关键帧 vs 非关键帧差距：LongVideoBench 与 Video-MME 最大（0.42），
│   MLVU 中等（0.65），MVBench 收窄——时间纯度最高的 benchmark 最难
└── 帧级采样丢失时间连贯性；F2C 用 key clips 替代 key frames：
    Video-MME +8.1%、LongVideoBench +5.6%、MLVU +10.3%

来源：VISTA 训练数据影响研究（对我们的数据策略最关键）
├── 长视频数据微调：Video-MME +3.7、MLVU +3.3、LongVideoBench +1.9
├── 再加短视频数据：无额外增益，甚至略伤长视频表现
└── 结论：长视频能力只能用长视频数据补 → 我们的数据生产方向
    （长视频/跨段推理样本）正确，且要防"难度坍缩"稀释
```

→ **对我们 10.4 根因 C 的意义**：时间交叉引用/跨段推理类 bad case 走"补长视频难度分级样本"，这正是 VLM 训练演进笔记里"数据难度 = 能力演进顶层的信号"的实证。

#### 案例 5：跨解析器/跨版本不可比（佐证验证记录表原则）

```text
来源：案例 1 的 verl 数据（同一模型 0 → 0.95）
├── 解析器版本换 = 分数不可比 → 呼应 vlm-eval-feedback-loop 的
│   "同评测集新旧对比，不跨版本比分数"
└── 也呼应知识树 Model Upgrade Adaptation：分版本 Baseline
```

#### 策略反馈报告可直接引用的外部依据（摘要版）

```text
1. 解析器假失败普遍性：verl #2150 / lm-eval-harness #2707、#3214 /
   36 模型研究（1/5 正确回答被格式误判）
2. 格式 vs 能力分离：Revealing Latent Reasoning（311 倍格式效应 +
   "格式作 gate 不作 reward"警示）
3. 计数短板：BLINK Counting（人类 93.75% vs 开源 10.83%）/
   TallyQA / Vision language models are blind（解码瓶颈）
4. 长视频短板：Video-MME（81.7% → 67.4%）/ TempCore（帧敏感 0.42）/
   VISTA（长视频数据才补长视频能力）
```
```